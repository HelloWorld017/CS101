...
class City:
    ...
    def get_monsoon_season(self):
        ...
        for weather in [
            *self.weathers,
            Weather(self.weathers[-1].date, 'Undefined', 0, 0, 0, 0)
        ]:
            if monsoon_start == None and weather.description == 'Rainy':
                monsoon_start = weather.date
            
            elif monsoon_start != None and weather.description != 'Rainy':
                monsoon_end = weather.date - timedelta(days=1)
                delta = (monsoon_end - monsoon_start).days + 1
                
                if delta > max_monsoon['length']:
                    max_monsoon['start'] = monsoon_start
                    max_monsoon['end'] = monsoon_end
                    max_monsoon['length'] = delta
                
                monsoon_start = None
        
        print(max_monsoon)
        if max_monsoon['length'] < 5:
            return "There is no monsoon season."
        
        return max_monsoon['start'], max_monsoon['end']
...
"""
provinces = task4_2_1()
print(provinces[0].cities[4].name, provinces[0].cities[4].get_monsoon_season())
print(provinces[4].cities[0].name, provinces[4].cities[0].get_monsoon_season())
"""
